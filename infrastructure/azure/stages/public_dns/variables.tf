...
variable "public_dns_resource_group_name" {
  description = "Optional explicit resource group name for retained public DNS resources. When null, a deterministic shared name is used."
  type        = string
  default     = null

  validation {
    condition     = var.public_dns_resource_group_name == null || can(regex("^[A-Za-z0-9._() -]{1,90}$", var.public_dns_resource_group_name))
    error_message = "public_dns_resource_group_name must be a valid Azure resource group name when supplied."
  }
}

variable "product_dns_zone_name" {
  description = "Delegated Azure DNS zone for first-release product hostnames."
  type        = string
  default     = "tokenobs.consultwithcloud.com"

  validation {
    condition     = var.product_dns_zone_name == "tokenobs.consultwithcloud.com"
    error_message = "product_dns_zone_name must remain tokenobs.consultwithcloud.com for first release."
  }
}
