...
Guarded Terraform Apply may run from GitHub Actions only after repository, actor, environment, region, derived workspace, branch, OIDC, least-privilege, and environment-protection checks pass. The customer organization slug defaults to `internal` and remains overrideable for non-internal customer scopes. Retained public DNS apply is fixed to `public_dns` in `pd_eastus2_internal`, uses protected environment approval, applies a same-run saved plan artifact, emits Cloudflare delegation output only, and verifies public NS delegation.
