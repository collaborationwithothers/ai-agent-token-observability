...
- `scripts/terraform-app-runtime-images.sh` lists successful ACR Image Publish runs for a workspace and dispatches `.github/workflows/terraform-plan.yml`. The workflow derives the commit SHA from the selected publish run and can auto-select the latest successful matching run when `app_runtime` is included.
