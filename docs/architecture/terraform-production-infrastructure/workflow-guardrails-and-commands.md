...
The normal Terraform deploy workflow produces a saved plan artifact for each selected stage, then applies that exact same-run artifact. The default `apply_mode` is `auto`, which applies the saved plan immediately after the stage plan succeeds. When `apply_mode` is `reviewed`, a stage-specific approval gate waits on the `terraform-apply` GitHub environment before the saved-plan apply job starts. In an all-stage deploy, the workflow plans and applies one stage before moving to the next dependent stage, so downstream stages can read remote state outputs produced earlier in the same run.
...
- `apply_mode`, defaulting to `auto`, with `auto` and `reviewed` as the only valid options
...
- The workflow validates `apply_mode`. `auto` applies saved plan artifacts without a GitHub approval gate. `reviewed` inserts a `terraform-apply` environment approval gate before each stage apply.
- The workflow does not require a manual confirmation phrase because the workspace is derived from validated inputs and apply mode is an explicit dispatch input.
...
- Reviewed-mode approval gate jobs must use the `terraform-apply` GitHub environment with required reviewers. Saved-plan apply jobs must depend on the approval gate in reviewed mode and must skip it in auto mode.
...
- Apply must use the exact saved plan artifact from the guarded plan job for the same stage and same run.
...
- The validator must fail if the normal Terraform deploy workflow lacks the default `auto` apply mode, lacks the reviewed `terraform-apply` approval gate, can bypass that gate in reviewed mode, applies without downloading the saved plan artifact, or includes `public_dns` in normal target stages.
...
The remote apply flow is implemented by guarded per-stage apply jobs in `.github/workflows/terraform-plan.yml`. Each apply job must run after its matching plan job, download the same-run plan artifact, select the derived workspace, and apply the saved plan file. In default `auto` mode, the apply job starts as soon as the saved plan artifact is available. In `reviewed` mode, a stage-specific approval gate waits for the `terraform-apply` environment before the apply job starts. In all-stage mode, the next stage plan job depends on the previous stage apply job.
...
- Never apply without a saved plan artifact from the same workflow run.
- Never apply a newly generated plan in the apply job when a saved plan artifact is required.
