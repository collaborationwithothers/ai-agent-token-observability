...
required_helper_patterns = {
    ...
    "Grafana endpoint top-level query": r'--query\s+"endpoint \|\| properties\.endpoint"',
    "Grafana Entra token acquisition": r'az\s+account\s+get-access-token[\s\S]*--resource\s+"\$\{GRAFANA_ENTRA_AUDIENCE\}"',
    "Grafana provider auth env": r'export\s+GRAFANA_AUTH="\$\{token\}"',
    "Grafana provider headers unset": r'unset\s+GRAFANA_HTTP_HEADERS',
